...
impl Parser {
    ...
    fn var_declr(&mut self) -> Result<Declr, ErrorHandler> {
        ...
        match operator.as_str() {
            ...
            ";" => return Ok(Declr::VarDeclr(id, Stmt::Expression(Expr::Literal(Lit::Nil)))),
            _ => {
                return Err(ErrorHandler::InvalidSyntax {
    ...
    fn statement(&mut self) -> Result<Stmt, ErrorHandler> {
        ...
        if matches!(tk_type.as_str(), "PRINT") {
            ...
            let mut else_st: Stmt = Stmt::Expression(Expr::Literal(Lit::Nil));
            ...
            self.expect("for")?;
            self.expect("(")?;
            let mut start_val: Declr = Declr::Reg(Stmt::Expression(Expr::Literal(Lit::Nil)));
            ...
            self.expect(";")?;
            return Ok(Stmt::Expression(result));
        }
    }
